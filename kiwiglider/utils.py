...
def dm2dd(degrees,minutes):
    ...
    decimal_degrees = np.where(degrees<0 or minutes<0,decimal_degrees*-1,decimal_degrees)
...
def temporary_cpt(palette=None,num_colors=None,seed=1,background=None):
    ...
    Input either a palettable palette name or an integer of distinguishable colors
    (with optional seed and background color list)

    Inputs:
    palette     :  str, palettable class path (ex colorbrewer.sequential.Blues_9)
    ...
    if palette:
        ...
        cpt = getattr(import_module('.'.join(['palettable']+palette[:-1])),palette[-1]).mpl_colormap
    elif num_colors:
        ...
        if type(background) is str:
            ...
            background = getattr(import_module('.'.join(['palettable']+background[:-1])),background[-1]).mpl_colors
        elif type(background) is list or background is None:
            pass
        else:
            raise TypeError('Input "background" must be a list or string')

        cpt = get_colormap(get_colors(num_colors,rng=seed,exclude_colors=background))
    else:
...
def do_step(step=0,**kwargs):
    ...
    also include required arguments for step to run [or should we make a mechanism to ask?]
    """
    match str(step):
        case '0':
...
class DeploymentYAML():
    """
    class with methods to add metadata and write out deployment-specific YAML for pyglider
    """
    ...
    def add_metadata(self,metadata=None):
        ...
        acknowledge = 'This work supported by funding from ' + self.excel_metadata['owner']
        self._add_meta('acknowledgement',acknowledge)
        pilot = self.excel_metadata['pilot']
        contributor_name = ','.join([self.excel_metadata['principal_investigator'],self.excel_metadata['data_manager'],pilot])
        self._add_meta('contributor_name',contributor_name)
        contributor_role = ','.join(['Principal Investigator'] + ['Data Manager'] + ['Operator']*int(pilot.count(',')+1))
...
class DeploymentNetCDF():
    ...
    def L0(self,style=None,search=None,profile_filt_time=None,profile_min_time=None,l0timeseries_directory='L0-timeseries',l0profile_directory='L0-profiles'):
        """
        ...
        if self.l0profile_directory:
            ...
            ncprocess.extract_timeseries_profiles(self.l0timeseries_outname,self.l0profile_directory,self.deployment_yaml)
    
    def L1(self,l1timeseries_directory='L1-timeseries',l1profile_directory='L1-profiles'):
        """
        ...
        # #perform qartod tests on L0 timeseries netcdf file to create L1 timeseries netcdf file
        #open dataset
        with xr.open_dataset(self.l0timeseries_outname) as inds:
            #output progress
            _log.info(f'Running QARTOD tests on {self.l0timeseries_outname}')
            #run initial tests
            xs = XarrayStream(inds, time='time', z='depth', lat='latitude', lon='longitude')
            ...
            with xr.open_dataset(self.l1timeseries_outname) as ds:
                ds = ds.swap_dims({'time_dim':'time'}).drop_vars(['z','lon','lat','crs','station'])
            ...
            for variable in outds.data_vars:
                if '_qc' in variable or '_qartod' in variable:
                    ...
                    if '_qc' in variable:
                        ...
                        outds[variable] = outds[variable].assign_attrs(
                            _FillValue=int(-127),
                            flag_values=[int(x) for x in range(1,5)] + [int(9)],
                            valid_max=int(9),
                            valid_min=int(0)
                        )
                else:
                    outds[variable] = outds[variable].astype('double').assign_attrs(
                        _FillValue=-999.0
                    )
                outds[variable] = outds[variable].fillna(outds[variable].attrs['_FillValue'])
        ...
        if self.l1profile_directory:
            ...
            ncprocess.extract_timeseries_profiles(self.l1timeseries_outname,self.l1profile_directory,self.deployment_yaml)
    ...
    def create_summary(self,timeseries_file,output_file,author='Anonymous',extra_text=None,map_bounds=None,
        ...
        [sci_vars.remove(v) for v in ['heading','pitch','roll','pressure','water_velocity_eastward','water_velocity_northward','distance_over_ground','profile_index','profile_direction']]
        ...
        for dy,var,s in zip(yshifts,plots[::-1],xaxis):
            ...
            pygmt.makecpt(cmap=temporary_cpt(palette=var['cmap']),series=[np.nanmin(z),np.nanmax(z)])
            fig.shift_origin(yshift=f'f{dy}')
            fig.basemap(region=region,projection=projection,frame=frame+[f'We{s}n+ggray90'])
            fig.plot(
                x=data['time'].values,
                y=data['depth'].values,
                fill=z,cmap=True,
        ...
        fig.basemap(region=[0,1,0,len(snapshot[0])+1],projection='X7/-9.5',frame='lbtr+ggray95')
        for col,text in enumerate(snapshot):
            fig.text(x=np.ones(len(text))*0.325*col,y=range(1,len(text)+1),text=text,justify='LM',offset='0.25/0',font=6)
        pygmt.makecpt(cmap=temporary_cpt(palette='cmocean.sequential.Amp_20'),series=[min_time,max_time])
        fig.colorbar(cmap=True,frame=['af','x+lTime'],position='jBL+w9.5+o-1/0+ma')
        ...
        fig.text(x=0,y=0,text=f'Prepared by {author} on {date}',justify='MC')
        if extra_text is not None:
